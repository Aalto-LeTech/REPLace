package fi.aalto.cs.replace.ui.repl;

import static fi.aalto.cs.replace.utils.PluginResourceBundle.getText;
...
//import fi.aalto.cs.replace.ui.IconListCellRenderer;
...
public class ReplConfigurationForm extends JPanel {
    ...
    public ReplConfigurationForm(@NotNull ReplConfigurationFormModel model, @Nullable Project project) {
        ...
        dontShowThisWindowCheckBox
                .setSelected(PluginSettings.shouldShowReplConfigurationDialog());

        infoTextLabel.setText(getText("ui.repl.configuration.form.infoLabel"));

        addFileChooser(workingDirectoryField, model.getProject());
        workingDirectoryField.setText(model.getModuleWorkingDirectory());

        Arrays.stream(model.getModuleNames()).forEach(moduleName -> moduleComboBox.addItem(moduleName));
...
//        moduleComboBox.setRenderer(new IconListCellRenderer<>(PluginIcons.A_PLUS_MODULE));
    ...
    public void updateModel() {
        ...
        PluginSettings.shouldShowReplConfigurationDialog_$eq(chosenState);;
    }
